...
export type PersonalityMode = "coach" | "empathetic" | "strategic" | "creative" | "roleplay" | "developer" | "focus";
...
export const personalityModes: Record<PersonalityMode, PersonalityMatrix> = {
    ...
    developer: {
        mode: "developer",
        intensity: 100,
        adaptationTriggers: ["break", "adjustments", "files", "structured", "code", "dev mode", "debug"],
        communicationStyle: {
...
export class PersonalityDetectionEngine {
    static detectOptimalMode(
        ...
        const focusTrigger = /(focus|stay on topic|be concise|deep dive)/.test(message);
        if (focusTrigger) {
            return 'focus';
        }
        ...
        if (sentiment === 'positive' && complexity === 'high') scores.strategic += 1;
        if (message.includes('how to') || message.includes('what should')) scores.coach += 1;
        if (message.includes('why') || message.includes('what if')) scores.creative += 1;

        if (conversationContext && conversationContext.length > 0) {
            const recentContext = conversationContext.slice(-3).join(' ').toLowerCase();
            if (recentContext.includes('strategic') || recentContext.includes('plan')) scores.strategic += 1;
            if (recentContext.includes('creative') || recentContext.includes('idea')) scores.creative += 1;
        }

        const topMode = Object.keys(scores).reduce((a, b) => (scores[a as PersonalityMode] > scores[b as PersonalityMode] ? a : b)) as PersonalityMode;
    ...
    private static scorePatterns(message: string, patterns: RegExp[]): number {
        return patterns.reduce((score, pattern) => score + (message.match(pattern) ? 1 : 0), 0);
    }
    ...
    private static detectUrgency(message: string): 'low' | 'medium' | 'high' {
        if (/(urgent|emergency|asap|immediately|critical)/.test(message)) return 'high';
...
export class ResponseGenerator {
    static generateResponse(context: ResponseContext): string {
        ...
        return this.craftPersonalizedResponse(context.personalityMode, context.userMessage, context.userEmotionalState, context.urgencyLevel);
    }
    ...
    private static craftPersonalizedResponse(
        ...
        switch (mode) {
            case 'coach': return this.generateCoachResponse(userMessage, emotionalState, urgency);
            case 'empathetic': return this.generateEmpatheticResponse(userMessage, emotionalState, urgency);
            case 'strategic': return this.generateStrategicResponse(userMessage, emotionalState, urgency);
            case 'creative': return this.generateCreativeResponse(userMessage, emotionalState, urgency);
            case 'focus': return this.generateFocusResponse(userMessage);
            default: return this.generateEmpatheticResponse(userMessage, emotionalState, urgency);
        }
    }

    private static generateFocusResponse(message: string): string {
        const objective = message.replace(/(focus on|stay on topic|deep dive|be concise)/i, '').trim();
        if (objective) {
    ...
    private static generateCoachResponse(message: string, emotional?: string, urgency?: string): string { return "Coach response placeholder"; }
    private static generateEmpatheticResponse(message: string, emotional?: string, urgency?: string): string { return "Empathetic response placeholder"; }
    private static generateStrategicResponse(message: string, emotional?: string, urgency?: string): string { return "Strategic response placeholder"; }
    private static generateCreativeResponse(message: string, emotional?: string, urgency?: string): string { return "Creative response placeholder"; }

    private static validateEthicalCompliance(context: ResponseContext): void {
        // Basic ethical validation logic
        const harmfulPatterns = [/(harm|hurt|kill|suicide|self-harm)/i, /(illegal|fraud|scam|hack)/i, /(discriminat|racist|sexist|hate)/i];
...
export class LearningEngine {
    static analyzeInteraction(
        ...
        if (!chosenMode) {
            console.error(`Error: LearningEngine could not find mode: ${conversationContext.personalityMode}`);
            return;
        }
