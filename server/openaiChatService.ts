...
export async function generateOpenAIResponse(
  ...
  conversationHistoryOrContext: Array<{ role: 'user' | 'assistant'; content: string }> | PersonalityContext = [],
  ...
  try {
    ...
    if (!client) {
      return { content: 'OpenAI API key not configured', success: false, error: 'Missing OPENAI_API_KEY' };
    }
    ...
    if (Array.isArray(conversationHistoryOrContext)) {
      ...
      if (typeof userNameOrMaxTokens === 'string') userName = userNameOrMaxTokens;
      if (typeof userNameOrMaxTokens === 'number') maxTokens = userNameOrMaxTokens;
    ...
    if (!content) {
      return { content: "I couldn't generate a response right now.", success: false, error: 'No content' };
    }

    return { content: String(content).trim(), success: true };
  } catch (err: any) {
    console.error('OpenAI chat error:', err);
    return { content: "I'm having trouble connecting to OpenAI right now.", success: false, error: err?.message || String(err) };
  }
}
