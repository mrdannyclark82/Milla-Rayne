...
export async function generateImage(
  ...
  if (!process.env.XAI_API_KEY) {
    return {
      success: false,
      error:
        'XAI API key is not configured. Please set XAI_API_KEY in your environment.',
    };
  }
  try {
    // Try the XAI image endpoint with a couple of retries and flexible response parsing
    const endpoint =
      process.env.XAI_IMAGE_ENDPOINT ||
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        ...
        if (
          ...
          data.output[0] &&
          (data.output[0].image_url || data.output[0].url)
        ) {
          return {
            success: true,
            imageUrl: data.output[0].image_url || data.output[0].url,
          };
        }
        ...
        if (
          ...
          data.images[0] &&
          (data.images[0].url || data.images[0].imageUrl)
        ) {
          return {
            success: true,
    ...
    return {
      success: false,
      error: lastError || 'XAI did not return an image URL.',
    };
  } catch (error) {
    return {
      success: false,
      error:
        error instanceof Error
          ? error.message
          : 'Unknown error during XAI image generation.',
    };
  }
}
