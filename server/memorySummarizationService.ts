 ...
 */
export async function generateMemorySummaries(userId: string): Promise<MemorySummary[]> {
  ...
  const userMemories = memoryCore.entries.filter(entry => entry.speaker === 'user' || entry.speaker === 'milla');
  ...
  for (const entry of userMemories) {
    ...
    const commonTopics = entryTopics.filter(topic => currentTopics.includes(topic));

    // Start a new group if current group is too large or topics diverge significantly
    if (currentGroup.length >= 10 || (currentGroup.length > 0 && commonTopics.length === 0 && entryTopics.length > 0)) {
  ...
  for (const group of groupedMemories) {
    const combinedContent = group.map(entry => `[${entry.speaker}]: ${entry.content}`).join('\n');
    ...
    try {
      ...
      if (aiResponse.success && aiResponse.content) {
        ...
        const summaryTopics = Array.from(new Set(group.flatMap(entry => entry.topics || [])));
        ...
        const newSummary: InsertMemorySummary = {
          ...
          emotionalTone: summaryEmotionalTone as 'positive' | 'negative' | 'neutral',
        };
 ...
 */
export async function retrieveMemorySummaries(userId: string, query: string, limit: number = 5): Promise<MemorySummary[]> {
