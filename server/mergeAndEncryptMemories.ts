...
async function main() {
    ...
    if (forceMode) {
        ...
        console.log(`✓ Updated ${path.basename(MEMORIES_FILE)} with merged content`);
        ...
        console.log('\n=== Dry Run Summary ===');
        console.log(`Would merge ${allMemories.length} entries into ${uniqueMemories.length} unique memories`);
...
function parseMemoryContent(content: string): MemoryEntry[] {
    ...
    for (const line of lines) {
        ...
        if (timestampMatch) {
            ...
            const altMatch = trimmed.match(/^(\d{4}-\d{2}-\d{2}[T\s]\d{2}:\d{2}:\d{2}[.\d]*Z?)\s*[:-]\s*(.+)$/);
            if (altMatch) {
