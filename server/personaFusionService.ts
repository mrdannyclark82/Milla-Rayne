...
import { getAmbientContext, type AmbientContext, getCurrentDateTime } from './realWorldInfoService';
...
export async function generateActivePersona(
  ...
  try {
    if (currentMessage) {
      const memoryContext = await getSemanticMemoryContext(currentMessage, userId);
  ...
  const ambientContextData = {
    ...
    motion: ambientData?.motionState && ambientData.motionState !== 'unknown' ? ambientData.motionState : undefined,
    lightLevel: ambientData?.lightLevel !== undefined 
...
function extractEmotionalPatterns(memoryContext: string): string[] {
  ...
  const emotionalKeywords = {
    positive: ['happy', 'excited', 'joy', 'grateful', 'love', 'wonderful', 'great'],
...
function synthesizePersonaSummary(data: {
  ...
  parts.push(`Current time: ${data.ambient.timeOfDay} on ${data.ambient.dateInfo}`);
  ...
  if (data.memory.emotionalPatterns.length > 0) {
    parts.push(`Emotional patterns: ${data.memory.emotionalPatterns.join(', ')}`);
  }
