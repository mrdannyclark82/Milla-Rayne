...
export async function saveInsightToMemory(
  ...
  const content = `[YouTube Insight] ${videoTitle} (${insight.category}): ${insight.content}`;

  await updateMemories(content, userId);

  console.log('💾 Saved insight to memory:', content);
}
