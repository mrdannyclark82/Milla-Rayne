...
export async function runTask(task: AgentTask): Promise<void> {
    try {
        ...
        if (task.metadata?.requireUserApproval && !task.metadata?.approved) {
            ...
            await updateTask(task.taskId, { status: 'failed', result: { error: errorMsg } });
            await logAuditEvent(task.taskId, task.agent, task.action, 'failed', errorMsg);
        ...
        if (!agent) {
            ...
            await logAuditEvent(task.taskId, task.agent, task.action, 'failed', 'Agent not found');
            return;
        }
