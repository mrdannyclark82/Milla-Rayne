...
async function migrateMemories() {
  ...
  try {
    ...
    for (const msg of messages) {
      try {
        ...
        if (lastTimestamp && currentSessionId) {
          const timeDiff = (messageTimestamp.getTime() - lastTimestamp.getTime()) / (1000 * 60);
          if (timeDiff > 30) {
            // End current session
            ...
            console.log(`Started new session after ${Math.round(timeDiff)} minute gap`);
          }
        }
    ...
    console.log(`Average session length: ${Math.round(stats.averageSessionLength)} minutes`);
    console.log(`Average time between sessions: ${Math.round(stats.averageTimeBetweenSessions)} minutes`);
    console.log(`Total messages: ${stats.totalMessages}`);
    console.log(`Average messages per session: ${Math.round(stats.averageMessagesPerSession)}`);
    
    // Show usage patterns
    const patterns = await storage.getUsagePatterns(DEFAULT_USER_ID);
    console.log('\n=== Top 5 Usage Patterns ===');
    patterns.slice(0, 5).forEach(p => {
      console.log(`${p.dayOfWeek} at ${p.hourOfDay}:00 - ${p.sessionCount} sessions, ${p.messageCount} messages`);
    });

    // Create backup of original file
    const backupPath = MEMORY_FILE_PATH + '.migrated-backup-' + new Date().toISOString().replace(/:/g, '-');
